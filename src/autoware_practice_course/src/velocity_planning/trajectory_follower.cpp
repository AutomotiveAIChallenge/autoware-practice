...
void SampleNode::update_current_state(const Odometry & msg)
{
  ...
  current_position_ = msg.pose.pose.position;  // 現在の車両の位置を更新する
  current_orientation_ = msg.pose.pose.orientation;  // 現在の車両の位置を更新する
};
...
double SampleNode::lateral_controller()
{
  ...
  double alpha = std::atan2(dy, dx) - calculateYawFromQuaternion(current_orientation_);
...
double SampleNode::calculateYawFromQuaternion(const geometry_msgs::msg::Quaternion& q) {
    // クォータニオンをオイラー角に変換
