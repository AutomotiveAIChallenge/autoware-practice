...
std::vector<SampleNode::Trajectory> SampleNode::create_trajectory_set()
{
  ...
  RCLCPP_INFO(this->get_logger(), "Target trajectory point set created. Size: %zu", target_trajectory_point_set.size());
  ...
  for (const auto & target_trajectory_point : target_trajectory_point_set) {
    ...
    RCLCPP_INFO(
      this->get_logger(), "Trajectory candidate created. Number of points: %zu", trajectory_candidate.points.size());
...
std::vector<autoware_auto_planning_msgs::msg::TrajectoryPoint> SampleNode::create_target_state_set()
{
  ...
  if (reference_trajectory_.points.empty()) {
    ...
    RCLCPP_INFO(this->get_logger(), "Closest waypoint index: %zu, Distance: %f", closest_waypoint_index, min_distance);
    ...
    for (int n = -4; n <= 5; ++n) {
      ...
      RCLCPP_INFO(
        this->get_logger(), "Added target state: index %d, position (%f, %f, %f)", n, target_state.pose.position.x,
        target_state.pose.position.y, target_state.pose.position.z);
    }
  ...
  const std::vector<SampleNode::Trajectory> & trajectory_set, const std::vector<std::vector<float>> & costmap)
{
  ...
  RCLCPP_INFO(this->get_logger(), "Best trajectory selected: %d with cost %f", min_index, trajectory_cost[min_index]);
